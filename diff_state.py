...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--table", "-t", required=True, choices=table_choices, help="Choice target state table"
    )
    ...
    parser.add_argument(
        "--src-env", "-se", required=True, choices=env_choices, help="Choose source env"
    )
    parser.add_argument("--src-snapshot", "-ss", required=True, type=str, help="Source snapshot name")
    # target snapshot (destination)
    parser.add_argument(
        "--dst-env", "-de", required=True, choices=env_choices, help="Choose destination env"
    )
